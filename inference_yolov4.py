...
original_image = cv2.imread("yolov4/test_data_set_0/sofa.jpeg")
...
try:
    sess = ort.InferenceSession("yolov4/yolov4.onnx", providers=[ 'AzureExecutionProvider, CPUExecutionProvider'])
...
def get_anchors(anchors_path, tiny=False):
    ...
    with open(anchors_path) as f:
        anchors = f.readline()
    anchors = np.array(anchors.split(',')).astype(np.float32)
...
def draw_bbox(image, bboxes, classes=read_class_names("yolov4/coco.names"), show_label=True):
    """
...
ANCHORS = "yolov4/yolov4_anchors.txt"
STRIDES = [8, 16, 32]
